...
impl GraphQuery {
    ...
    pub fn search_symbols_with_inheritance(
        &self,
        pattern: &str,
        symbol_types: Option<Vec<NodeKind>>,
        ...
        if let Some(filters) = inheritance_filters {
            results.retain(|symbol_info| {
                filters
                    .iter()
                    .any(|filter| self.matches_inheritance_filter(&symbol_info.node, filter))
            });
        }
            ...
            if matches!(edge.kind, EdgeKind::Extends) {
                if let Some(parent_node) = self.graph.get_node(&edge.target) {
                    ...
                    base_classes.push(InheritanceRelation {
                        ...
                        relationship_type: if is_metaclass {
                            "metaclass".to_string()
                        } else {
                            "extends".to_string()
                        },
    ...
    pub fn is_metaclass(&self, node_id: &NodeId) -> Result<bool> {
        if let Some(node) = self.graph.get_node(node_id) {
            ...
            let name_suggests_metaclass =
                node.name.contains("Meta") || node.name.ends_with("Metaclass");
    ...
    fn collect_inheritance_chain_recursive(
        &self,
        node_id: &NodeId,
...
mod tests {
    ...
    fn test_symbol_search_regex() {
        ...
        let guild_manager_agent_node = create_test_node(
            "GuildManagerAgent",
            NodeKind::Class,
            "guild_manager_agent.py",
        );
