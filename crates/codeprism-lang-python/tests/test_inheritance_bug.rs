...
"#;
    ...
    let parent_call_nodes: Vec<_> = call_nodes.iter().filter(|n| n.name == "Parent").collect();

    if parent_call_nodes.is_empty() {
        println!("❌ No Call node found for Parent inheritance");
    } else {
        println!(
            "✅ Found {} Call node(s) for Parent inheritance",
            parent_call_nodes.len()
        );
    }
...
"#;
    ...
    let guild_manager = class_nodes
        .iter()
        .find(|n| n.name == "GuildManagerAgent")
        .unwrap();
    let guild_manager_edges: Vec<_> = result
        .edges
        .iter()
    ...
    for edge in &guild_manager_edges {
        if let Some(target_node) = result.nodes.iter().find(|n| n.id == edge.target) {
            println!(
                "  GuildManagerAgent -> {} (kind: {:?})",
                target_node.name, edge.kind
            );
        }
    }

    // Check for Agent inheritance Call node
    let agent_call_nodes: Vec<_> = call_nodes.iter().filter(|n| n.name == "Agent").collect();

    if agent_call_nodes.is_empty() {
        println!("❌ No Call node found for Agent inheritance");
    } else {
        println!(
            "✅ Found {} Call node(s) for Agent inheritance",
            agent_call_nodes.len()
        );
    }
