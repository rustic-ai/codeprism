...
impl McpServer {
    ...
    pub fn new() -> Result<Self> {
        ...
        let resource_manager = ResourceManager::new(codeprism_server.clone());
        let tool_registry = ToolRegistry::new(codeprism_server.clone());
        let prompt_manager = PromptManager::new(codeprism_server.clone());
    ...
    pub fn new_with_config(
        ...
        let codeprism_server = Arc::new(RwLock::new(CodePrismMcpServer::new_with_config(
            memory_limit_mb,
            batch_size,
            max_file_size_mb,
    ...
    pub async fn run_stdio(self) -> Result<()> {
        info!("Starting CodePrism MCP server with stdio transport");
...
mod tests {
    ...
    async fn test_mcp_server_creation() {
        ...
        assert_eq!(server.server_info().name, "codeprism-mcp");
...
"#,
    ...
    async fn test_server_with_repository_initialization() {
        ...
        let codeprism_server = server.codeprism_server.read().await;
        assert!(codeprism_server.repository_path().is_some());
    }
    ...
    async fn test_full_mcp_workflow() {
        ...
        assert!(uris.iter().any(|uri| uri == "codeprism://repository/stats"));
        assert!(uris.iter().any(|uri| uri == "codeprism://graph/repository"));
    ...
    async fn test_server_capabilities_validation() {
        let server = create_test_server_with_repository().await;
        let codeprism_server = server.codeprism_server.read().await;
        let capabilities = codeprism_server.capabilities();
