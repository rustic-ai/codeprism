...
impl ToolManager {
    ...
    async fn repository_stats(&self, server: &CodePrismMcpServer) -> Result<CallToolResult> {
    ...
    fn is_valid_dependency_node(&self, node: &codeprism_core::Node) -> bool {
    ...
    async fn find_dependencies(
        ...
        let dependency_type = match dependency_type_str {
            "direct" => codeprism_core::graph::DependencyType::Direct,
            "calls" => codeprism_core::graph::DependencyType::Calls,
            "imports" => codeprism_core::graph::DependencyType::Imports,
            "reads" => codeprism_core::graph::DependencyType::Reads,
            "writes" => codeprism_core::graph::DependencyType::Writes,
            _ => {
    ...
    fn parse_inheritance_filter(
        &self,
        filter_str: &str,
    ) -> Option<codeprism_core::InheritanceFilter> {
    ...
    fn parse_node_id(&self, hex_str: &str) -> Result<codeprism_core::NodeId> {
    ...
    async fn content_stats(&self, server: &CodePrismMcpServer) -> Result<CallToolResult> {
        let stats = server.content_search().get_stats();
    ...
    fn is_decorator_node(&self, node: &codeprism_core::Node) -> bool {
...
"#,
        ...
        let mut server = CodePrismMcpServer::new().expect("Failed to create server");
        server
...
"#,
        ...
        // Create a mock node
        let span = codeprism_core::ast::Span::new(0, 20, 2, 2, 1, 21);
        let node = codeprism_core::ast::Node::new(
            "test_repo",
            codeprism_core::ast::NodeKind::Function,
            "example_function".to_string(),
            codeprism_core::ast::Language::Python,
            test_file.clone(),
            span,
        );
    ...
    async fn detect_singleton_pattern(
        ...
        for class in classes {
            ...
            let has_private_constructor = methods.iter().any(|edge| {
                if let Some(target_node) = server.graph_store().get_node(&edge.target) {
                    target_node.kind == codeprism_core::NodeKind::Method
    ...
    async fn detect_observer_pattern(
        ...
        for class in classes {
            ...
            let events = server
                ...
                .iter()
                .filter(|edge| edge.kind == codeprism_core::EdgeKind::Emits)
                .count();
        ...
        for class in classes {
            ...
            for edge in methods {
                if let Some(method_node) = server.graph_store().get_node(&edge.target) {
                    if method_node.kind == codeprism_core::NodeKind::Method {
    ...
    async fn detect_protocol_pattern(
        ...
        for class in classes {
            ...
            let method_count = methods
                ...
                .filter(|edge| {
                    if let Some(method_node) = server.graph_store().get_node(&edge.target) {
                        method_node.kind == codeprism_core::NodeKind::Method
                    } else {
                        false
                    }
                })
        ...
        for kind in &node_kinds {
            ...
            for node in nodes {
                ...
                import_edges.extend(
                    edges
                        .into_iter()
                        .filter(|edge| edge.kind == codeprism_core::EdgeKind::Imports),
                );
            }
        }
    ...
    async fn detect_api_breaking_changes(
        ...
        for function in functions {
            ...
            let parameter_like_deps = dependencies
                .iter()
                .filter(|dep| matches!(dep.target_node.kind, codeprism_core::NodeKind::Variable))
                .count();
