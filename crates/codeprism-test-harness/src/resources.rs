    ...
    pub async fn test_resources_capability(&self) -> Result<ResourceTestResult> {
        ...
        for resource in &sample_resources {
            if let Err(e) = self.validator.validate_resource_metadata(resource) {
                result.validation_errors.push(format!("Resource metadata validation failed for {}: {}", resource.uri, e));
            } else {
        ...
        if let Err(e) = self.validator.validate_resource_content(&sample_content).await {
            result.validation_errors.push(format!("Content validation failed: {}", e));
        }
...
impl ResourceValidator {
    ...
    pub fn new() -> Self {
        ...
        mime_validators.insert("text/plain".to_string(), MimeType {
        ...
        mime_validators.insert("application/json".to_string(), MimeType {
        ...
        mime_validators.insert("application/octet-stream".to_string(), MimeType {
            main_type: "application".to_string(),
    ...
    pub async fn validate_resource_content(&self, content: &ResourceContents) -> Result<()> {
        ...
        if content_size > self.max_resource_size {
            return Err(anyhow!("Resource content exceeds maximum size: {} bytes", content_size));
        }
            ...
            match &mime_validator.encoding_validator {
                ...
                EncodingValidator::Base64 => {
                    if let Some(blob) = &content.blob {
                        ...
                        general_purpose::STANDARD.decode(blob)
...
impl Default for ResourceTestResult {
    fn default() -> Self {
        Self {
            success: false,
...
mod tests {
    ...
    fn test_resource_metadata_validation() {
        ...
        assert!(validator.validate_resource_metadata(&valid_resource).is_ok());
        ...
        assert!(validator.validate_resource_metadata(&invalid_resource).is_err());
    }
    ...
    async fn test_content_validation() {
        ...
        assert!(validator.validate_resource_content(&text_content).await.is_ok());
        ...
        assert!(validator.validate_resource_content(&binary_content).await.is_ok());
        ...
        assert!(validator.validate_resource_content(&invalid_json_content).await.is_err());
    }
