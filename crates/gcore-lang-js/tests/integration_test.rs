...
fn test_parse_imports() {
    ...
    let imports: Vec<_> = result
        .nodes
        .iter()
        .filter(|n| matches!(n.kind, gcore_lang_js::NodeKind::Import))
        .map(|n| &n.name)
        .collect();
    ...
    assert!(result
        .nodes
        .iter()
        .any(|n| n.name == "App" && matches!(n.kind, gcore_lang_js::NodeKind::Function)));
}
...
fn test_parse_typescript() {
    ...
    let lang = result
        .nodes
        .iter()
        .find(|n| matches!(n.kind, gcore_lang_js::NodeKind::Module))
        .map(|n| n.lang)
        .expect("Should have module node");

    assert_eq!(lang, gcore_lang_js::Language::TypeScript);

    // Should find the UserService class and its methods
    assert!(result
        .nodes
        .iter()
        .any(|n| n.name == "UserService" && matches!(n.kind, gcore_lang_js::NodeKind::Class)));
    assert!(result
        .nodes
        .iter()
        .any(|n| n.name == "getUser" && matches!(n.kind, gcore_lang_js::NodeKind::Method)));
    assert!(result
        .nodes
        .iter()
        .any(|n| n.name == "createUser" && matches!(n.kind, gcore_lang_js::NodeKind::Method)));

    // Should find function calls
    let calls: Vec<_> = result
        .nodes
        .iter()
        .filter(|n| matches!(n.kind, gcore_lang_js::NodeKind::Call))
        .map(|n| &n.name)
        .collect();
...
fn test_edge_extraction() {
    ...
    let call_edges: Vec<_> = result
        .edges
        .iter()
        .filter(|e| matches!(e.kind, gcore_lang_js::EdgeKind::Calls))
        .collect();
