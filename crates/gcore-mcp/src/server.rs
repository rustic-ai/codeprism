...
pub struct McpServer {
    ...
    /// Core GCore server instance
    gcore_server: Arc<RwLock<GCoreMcpServer>>,
    /// Resource manager
...
impl McpServer {
    ...
    pub fn new() -> Result<Self> {
        let gcore_server = Arc::new(RwLock::new(GCoreMcpServer::new()?));
    ...
    pub fn new_with_config(
        ...
        let gcore_server = Arc::new(RwLock::new(GCoreMcpServer::new_with_config(
            memory_limit_mb,
            batch_size,
            max_file_size_mb,
    ...
    pub async fn run_stdio(mut self) -> Result<()> {
        info!("Starting GCore MCP server with stdio transport");
    ...
    pub async fn run_with_transport<T: Transport>(mut self, mut transport: T) -> Result<()> {
        info!("Starting GCore MCP server");
...
"#).unwrap();
    ...
    async fn test_full_mcp_workflow() {
        ...
        assert!(uris.iter().any(|uri| uri == "gcore://repository/stats"));
        assert!(uris.iter().any(|uri| uri == "gcore://graph/repository"));
        ...
        assert_eq!(tools.tools.len(), 6); // All 6 tools should be available
        ...
        assert_eq!(prompts.prompts.len(), 5); // All 5 prompts should be available
