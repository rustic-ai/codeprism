...
impl ToolManager {
    ...
    async fn search_content(&self, server: &GCoreMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        let args = arguments.ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
    ...
    async fn find_files(&self, server: &GCoreMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        let args = arguments.ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
    ...
    async fn content_stats(&self, server: &GCoreMcpServer) -> Result<CallToolResult> {
        let stats = server.content_search().get_stats();
...
"#).unwrap();
    ...
    async fn test_list_tools() {
        ...
        assert_eq!(tools_result.tools.len(), 9);
