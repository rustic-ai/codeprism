...
use super::{
    ChunkId, ContentChunk, ContentNode, ContentType, DocumentFormat, ConfigFormat, CommentContext
};
...
impl ConfigParser {
    ...
    fn parse_xml(&self, file_path: &Path, content: &str) -> Result<Vec<ContentChunk>> {
        ...
        let tag_regex = Regex::new(r"<([^/>]+)>([^<]*)</\1>").unwrap();
        let mut chunks = Vec::new();
