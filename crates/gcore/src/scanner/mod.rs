...
impl RepositoryScanner {
    ...
    pub fn discover_files<P: AsRef<Path>>(&self, repo_path: P) -> Result<Vec<PathBuf>> {
        ...
        if !repo_path.exists() {
            return Err(Error::io(format!("Repository path does not exist: {}", repo_path.display())));
        }

        if !repo_path.is_dir() {
            return Err(Error::io(format!("Repository path is not a directory: {}", repo_path.display())));
        }
    ...
    fn should_exclude_directory(&self, dir_path: &Path, repo_root: &Path) -> bool {
        ...
        if let Ok(rel_path) = dir_path.strip_prefix(repo_root) {
            let path_components: Vec<&str> = rel_path.components()
            ...
            match self.dependency_mode {
                ...
                DependencyMode::IncludeAll => {
                    ...
                    let basic_excludes = [".git", "build", "dist", ".vscode", ".idea", "__pycache__"];
                    if let Some(current_dir_name) = path_components.last() {
        ...
        if let Some(dir_name) = dir_path.file_name().and_then(|n| n.to_str()) {
            match self.dependency_mode {
                ...
                DependencyMode::Smart => {
                    ...
                    let is_dependency = ["node_modules", "venv", ".venv", ".tox", "vendor"].contains(&dir_name);
                    if is_dependency {
    ...
    fn is_in_dependency_directory(&self, path_components: &[&str]) -> bool {
        ...
        path_components.iter().any(|&component| dependency_dirs.contains(&component))
    }
    ...
    fn should_exclude_dependency_directory(&self, path_components: &[&str]) -> bool {
        ...
        let dependency_dirs = ["node_modules", "venv", ".venv", ".tox", "vendor", "target"];
        if let Some(dep_index) = path_components.iter().position(|&c| dependency_dirs.contains(&c)) {
            ...
            let exclude_patterns = [
                ...
                "docs", "examples", "benchmarks", "node_modules",
    ...
    fn process_file<P: AsRef<Path>>(&self, file_path: P) -> Result<Option<DiscoveredFile>> {
        ...
        let metadata = std::fs::metadata(file_path)
            .map_err(|e| Error::io(format!("Failed to read metadata for {}: {}", file_path.display(), e)))?;
        ...
        if self.dependency_mode == DependencyMode::Smart {
            if let Some(repo_root) = file_path.ancestors().nth(10) { // Approximate repo root
    ...
    fn is_important_dependency_file(&self, file_path: &Path) -> bool {
        if let Some(file_name) = file_path.file_name().and_then(|n| n.to_str()) {
            ...
            let important_files = [
                "__init__.py", "index.js", "index.ts", "lib.rs", "main.rs",
                "package.json", "setup.py", "Cargo.toml", "requirements.txt"
            ];
            ...
            let internal_indicators = [
                "_internal", "_private", "internal", "private", 
                ".test.", ".spec.", "_test", "_spec"
            ];
            
            let path_str = file_path.to_string_lossy().to_lowercase();
            if internal_indicators.iter().any(|&indicator| path_str.contains(indicator)) {
                return false;
            }
