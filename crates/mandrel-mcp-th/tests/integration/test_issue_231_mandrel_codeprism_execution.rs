...
async fn test_result_structure_validation() {
    ...
    let result =
        execute_moth_cli(&["run", spec_path, "--output", output_dir.to_str().unwrap()]).await;
    ...
    match result {
        Ok(output) => {
            ...
            let output_file = output_dir.join("report.json");
            assert!(output_file.exists(), "Output file should be created");

            // Verify result structure (basic validation)
            let file_content = std::fs::read_to_string(&output_file).unwrap();
            assert!(
                file_content.contains("suite_name"),
                "Results should contain suite_name"
            );
            assert!(
                file_content.contains("total_tests"),
                "Results should contain total_tests"
            );
            assert!(
                file_content.contains("passed"),
