...
async fn start_codeprism_server() -> std::io::Result<Child> {
    ...
    cmd.args(["run", "--package", "codeprism-mcp-server", "--bin", "codeprism-mcp-server"])
        .env("RUST_LOG", "info")
...
async fn test_load_rust_comprehensive_specification() {
    ...
    assert!(Path::new(spec_path).exists(), "Rust comprehensive spec should exist");
    ...
    match result {
        Ok(output) => {
            ...
            assert!(!output.stdout.contains("error"), "Should not contain errors");
        }
        Err(e) => {
...
async fn test_execute_rust_comprehensive_against_codeprism_server() {
    ...
    match result {
        Ok(output) => {
            ...
            assert!(output.execution_time < Duration::from_secs(60), "Should complete within 60 seconds");
            ...
            println!("✅ Rust comprehensive execution completed: {} tests", output.total_tests().unwrap_or(0));
        }
        Err(e) => {
            // Expected to fail initially - this is RED phase
...
async fn test_execute_python_comprehensive_against_codeprism_server() {
    ...
    // Verify the specification file exists
    assert!(Path::new(spec_path).exists(), "Python comprehensive spec should exist");
...
async fn test_execute_java_comprehensive_against_codeprism_server() {
    ...
    assert!(Path::new(spec_path).exists(), "Java comprehensive spec should exist");
    ...
    match result {
        Ok(output) => {
            assert!(output.success, "CLI execution should succeed");
            if let Some(total) = output.total_tests() {
                assert_eq!(total, 19, "Java spec should have 19 tools");
            }
...
async fn test_execute_javascript_comprehensive_against_codeprism_server() {
    ...
    assert!(Path::new(spec_path).exists(), "JavaScript comprehensive spec should exist");
    ...
    match result {
        Ok(output) => {
            ...
            assert!(total_duration < Duration::from_secs(60), 
                "Execution should complete within 60 seconds, took: {:?}", total_duration);
            assert!(output.execution_time < Duration::from_secs(60), 
...
async fn test_comprehensive_tool_coverage() {
    ...
    let specs = [
        ...
        ("java", "../../crates/codeprism-moth-specs/codeprism/comprehensive/codeprism-java-comprehensive.yaml", 19),
    ...
    for (language, spec_path, expected_tools) in specs {
        ...
        match result {
            Ok(output) => {
                if let Some(total) = output.total_tests() {
                    assert_eq!(total, expected_tools, 
                        "{} spec should have {} tools, found {}", language, expected_tools, total);
                }
                
                if let Some(passed) = output.passed_tests() {
                    assert!(passed > 0, "{} spec should have at least some passing tests", language);
                }
                
                println!("✅ {} tool coverage validated: {} tools", language, expected_tools);
            }
            Err(e) => {
                println!("Expected {} coverage test failure during RED phase: {}", language, e);
            }
        }
    }
}
...
async fn test_result_structure_validation() {
    ...
    match result {
        Ok(output) => {
            ...
            assert!(file_content.contains("suite_name"), "Results should contain suite_name");
            assert!(file_content.contains("total_tests"), "Results should contain total_tests");
            assert!(file_content.contains("passed"), "Results should contain passed count");
            
            println!("✅ Result structure validation passed");
        }
        Err(e) => {
            println!("Expected result structure test failure during RED phase: {}", e);
        }
    }
}
...
async fn test_invalid_specification_handling() {
    ...
    match result {
        Ok(output) => {
            ...
            assert!(output.stderr.contains("not found") || output.stderr.contains("No such file"), 
...
async fn test_server_connection_failure_handling() {
    ...
    match result {
        Ok(output) => {
            ...
            if !output.success {
                assert!(output.stderr.contains("connection") || output.stderr.contains("server"), 
        ...
        Err(e) => {
            println!("Expected connection failure test failure during RED phase: {}", e);
        }
    }
} 