...
impl ContentSearchManager {
    ...
    fn is_source_code_language(&self, language: Language) -> bool {
        matches!(language, Language::JavaScript | Language::TypeScript | Language::Python | 
...
mod tests {
    ...
    fn test_language_detection() {
        ...
        assert_eq!(manager.detect_language(Path::new("test.js")), Some(Language::JavaScript));
        assert_eq!(manager.detect_language(Path::new("test.py")), Some(Language::Python));
        assert_eq!(manager.detect_language(Path::new("test.rs")), Some(Language::Rust));
        assert_eq!(manager.detect_language(Path::new("test.java")), Some(Language::Java));
        assert_eq!(manager.detect_language(Path::new("test.ts")), Some(Language::TypeScript));
    ...
    fn test_simple_search() {
        ...
        let _ = manager.index_file(Path::new("test1.md"), "# Hello World\n\nThis is a test document about programming.");
        let _ = manager.index_file(Path::new("test2.md"), "# Testing\n\nAnother document for testing purposes.");
    ...
    fn test_search_documentation() {
        ...
        // Index documentation
        let _ = manager.index_file(Path::new("doc.md"), "# API Documentation\n\nThis describes the API.");
        let _ = manager.index_file(Path::new("readme.txt"), "README file with API information.");
        let _ = manager.index_file(Path::new("code.js"), "// This is not documentation\nfunction api() {}");
    ...
    fn test_regex_search() {
        ...
        let _ = manager.index_file(Path::new("test.md"), "Email: user@example.com\nAnother: admin@test.org");
    ...
    fn test_search_in_files() {
        ...
        let results = manager.search_in_files("content", vec!["*.md".to_string()], Some(10)).unwrap();
        assert!(!results.is_empty());
    }
    ...
    fn test_search_query_builder() {
        ...
        let query = SearchQueryBuilder::new("search")
            .content_types(vec![ContentType::Documentation { format: DocumentFormat::Markdown }])
            .build();
