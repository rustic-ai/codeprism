...
impl PythonAnalyzer {
    ...
    fn initialize_patterns(&mut self) {
        ...
        let framework_decorators = vec![
            ...
            DecoratorPattern {
                ...
                pattern: Regex::new(r"@(?:login_required|permission_required|csrf_exempt)").unwrap(),
            ...
            DecoratorPattern {
                ...
                effects: vec!["Async task execution".to_string(), "Queue processing".to_string()],
                is_factory: true,
            },
        ];
        self.decorator_patterns.insert("framework".to_string(), framework_decorators);
        ...
        let pattern_decorators = vec![
            DecoratorPattern {
                ...
                framework: None,
                effects: vec!["Result caching".to_string(), "Performance optimization".to_string()],
                is_factory: false,
            },
            DecoratorPattern {
            ...
            DecoratorPattern {
                ...
                framework: None,
                effects: vec!["Access control".to_string(), "Security enforcement".to_string()],
                is_factory: false,
            },
            DecoratorPattern {
                ...
                framework: None,
                effects: vec!["Function logging".to_string(), "Execution tracing".to_string()],
                is_factory: false,
            },
        ];
        self.decorator_patterns.insert("patterns".to_string(), pattern_decorators);

        // Metaclass patterns
        let metaclass_patterns = vec![
            MetaclassPattern {
                name: "Registry Metaclass".to_string(),
                pattern: Regex::new(r"class\s+\w+\s*\([^)]*metaclass\s*=\s*\w*Registry\w*").unwrap(),
                impact: "Automatic class registration".to_string(),
            },
            MetaclassPattern {
                name: "Singleton Metaclass".to_string(),
                pattern: Regex::new(r"class\s+\w+\s*\([^)]*metaclass\s*=\s*\w*Singleton\w*").unwrap(),
        ...
        self.metaclass_patterns.insert("common".to_string(), metaclass_patterns);
    }
    ...
    fn extract_metaclass(&self, bases_str: &str) -> Option<String> {
        ...
        if let Some(captures) = metaclass_pattern.captures(bases_str) {
    ...
    pub fn get_python_recommendations(&self, decorators: &[DecoratorInfo], metaclasses: &[MetaclassInfo], inheritance: &[InheritanceInfo]) -> Vec<String> {
        ...
        if framework_decorators > 0 {
            recommendations.push("Consider documenting framework-specific decorator behavior.".to_string());
        }

        let factory_decorators = decorators.iter().filter(|d| d.is_factory).count();
        if factory_decorators > 0 {
            recommendations.push("Factory decorators detected - ensure proper parameter validation.".to_string());
        }

        // Metaclass recommendations
        if !metaclasses.is_empty() {
            recommendations.push("Metaclasses detected - document their behavior and impact on subclasses.".to_string());
            recommendations.push("Consider if metaclass functionality could be achieved with simpler patterns.".to_string());
        }

        // Inheritance recommendations
        let diamond_inheritance = inheritance.iter().filter(|i| i.has_diamond_inheritance).count();
        if diamond_inheritance > 0 {
            recommendations.push("Diamond inheritance detected - verify MRO behavior is as expected.".to_string());
        }

        let complex_inheritance = inheritance.iter().filter(|i| i.base_classes.len() > 2).count();
        if complex_inheritance > 0 {
            recommendations.push("Complex inheritance hierarchies detected - consider composition over inheritance.".to_string());
        }

        recommendations.push("Use type hints for better code documentation and IDE support.".to_string());
...
mod tests {
    ...
    fn test_mixin_identification() {
        ...
        let base_classes = vec!["BaseMixin".to_string(), "RegularClass".to_string(), "UtilMix".to_string()];
