...
impl LanguageParser for PythonParserAdapter {
    ...
    fn parse(&self, context: &ParseContext) -> prism_core::error::Result<ParseResult> {
        ...
        match prism_lang_python::parse_file(
                ...
                let nodes: Vec<Node> = py_nodes
                    ...
                    .map(|py_node| {
                        ...
                        let prism_kind = match py_node.kind {
                            prism_lang_python::NodeKind::Function => {
                                prism_core::ast::NodeKind::Function
                            }
                            prism_lang_python::NodeKind::Class => prism_core::ast::NodeKind::Class,
                            prism_lang_python::NodeKind::Variable => {
                                prism_core::ast::NodeKind::Variable
                            }
                            prism_lang_python::NodeKind::Module => {
                                prism_core::ast::NodeKind::Module
                            }
                            prism_lang_python::NodeKind::Import => {
                                prism_core::ast::NodeKind::Import
                            }
                            prism_lang_python::NodeKind::Parameter => {
                                prism_core::ast::NodeKind::Parameter
                            }
                            prism_lang_python::NodeKind::Method => {
                                prism_core::ast::NodeKind::Method
                            }
                            prism_lang_python::NodeKind::Call => prism_core::ast::NodeKind::Call,
                            prism_lang_python::NodeKind::Literal => {
                                prism_core::ast::NodeKind::Literal
                            }
                            prism_lang_python::NodeKind::Route => prism_core::ast::NodeKind::Route,
                            prism_lang_python::NodeKind::SqlQuery => {
                                prism_core::ast::NodeKind::SqlQuery
                            }
                            prism_lang_python::NodeKind::Event => prism_core::ast::NodeKind::Event,
                            prism_lang_python::NodeKind::Unknown => {
                                prism_core::ast::NodeKind::Unknown
                            }
                        };
                ...
                let edges: Vec<Edge> = py_edges
                    ...
                    .map(|py_edge| {
                        ...
                        let prism_edge_kind = match py_edge.kind {
                            ...
                            prism_lang_python::EdgeKind::Writes => {
                                prism_core::ast::EdgeKind::Writes
                            }
                            prism_lang_python::EdgeKind::Imports => {
                                prism_core::ast::EdgeKind::Imports
                            }
                            prism_lang_python::EdgeKind::Emits => prism_core::ast::EdgeKind::Emits,
                            prism_lang_python::EdgeKind::RoutesTo => {
                                prism_core::ast::EdgeKind::RoutesTo
                            }
                            prism_lang_python::EdgeKind::Raises => {
                                prism_core::ast::EdgeKind::Raises
                            }
                            prism_lang_python::EdgeKind::Extends => {
                                prism_core::ast::EdgeKind::Extends
                            }
                            prism_lang_python::EdgeKind::Implements => {
                                prism_core::ast::EdgeKind::Implements
                            }
                        };

                        // Convert NodeIds by using hex representation
                        let prism_source =
                            prism_core::ast::NodeId::from_hex(&py_edge.source.to_hex()).unwrap();
                        let prism_target =
                            prism_core::ast::NodeId::from_hex(&py_edge.target.to_hex()).unwrap();
            ...
            Err(e) => Err(prism_core::error::Error::parse(
                &context.file_path,
                ...
                let nodes: Vec<Node> = js_nodes
                    ...
                    .map(|js_node| {
                        ...
                        let prism_kind = match js_node.kind {
                            ...
                            prism_lang_js::NodeKind::SqlQuery => {
                                prism_core::ast::NodeKind::SqlQuery
                            }
                ...
                let edges: Vec<Edge> = js_edges
                    ...
                    .map(|js_edge| {
                        ...
                        let prism_edge_kind = match js_edge.kind {
                            ...
                            prism_lang_js::EdgeKind::RoutesTo => {
                                prism_core::ast::EdgeKind::RoutesTo
                            }
                            ...
                            prism_lang_js::EdgeKind::Implements => {
                                prism_core::ast::EdgeKind::Implements
                            }
                        };
                        ...
                        // Convert NodeIds by using hex representation
                        let prism_source =
                            prism_core::ast::NodeId::from_hex(&js_edge.source.to_hex()).unwrap();
                        let prism_target =
                            prism_core::ast::NodeId::from_hex(&js_edge.target.to_hex()).unwrap();
...
impl PrismMcpServer {
    ...
    pub fn new() -> Result<Self> {
        ...
        let content_search = Arc::new(prism_core::ContentSearchManager::with_graph_store(
            graph_store.clone(),
        ));
