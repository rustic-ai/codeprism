    ...
    match direction {
        "forward" | "both" => {
            if let Ok(dependencies) = server
                .graph_query()
                .find_dependencies(&node.id, prism_core::graph::DependencyType::Direct)
            {
                for dep in dependencies.iter().take(max_depth) {
                    ...
                    if include_transformations
                        && matches!(dep.edge_kind, prism_core::EdgeKind::Calls)
                    {
...
async fn collect_transitive_deps(
    ...
    if let Ok(dependencies) = server
        .graph_query()
        .find_dependencies(current_id, prism_core::graph::DependencyType::Direct)
    {
        for dep in dependencies {
