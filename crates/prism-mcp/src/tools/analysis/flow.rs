...
async fn trace_symbol_data_flow(
    ...
    match direction {
        "forward" | "both" => {
            if let Ok(dependencies) = server.graph_query().find_dependencies(&node.id, prism_core::graph::DependencyType::Direct) {
                for dep in dependencies.iter().take(max_depth) {
                    ...
                    if include_transformations && matches!(dep.edge_kind, prism_core::EdgeKind::Calls) {
...
async fn collect_transitive_deps(
    server: &PrismMcpServer,
    current_id: &prism_core::NodeId,
    transitive_deps: &mut Vec<Value>,
    visited: &mut std::collections::HashSet<prism_core::NodeId>,
    cycles: &mut Vec<Value>,
    max_depth: usize,
    current_depth: usize,
    detect_cycles: bool,
    root_id: &prism_core::NodeId,
) {
    ...
    if let Ok(dependencies) = server.graph_query().find_dependencies(current_id, prism_core::graph::DependencyType::Direct) {
        for dep in dependencies {
...
/// Trace data flow
async fn trace_data_flow(server: &PrismMcpServer, arguments: Option<&Value>) -> Result<CallToolResult> {
    ...
    let variable_or_parameter = args.get("variable_or_parameter")
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("Missing variable_or_parameter parameter (or start_symbol, symbol, target)"))?;
    ...
    let result = if let Ok(symbol_results) = server.graph_query().search_symbols(variable_or_parameter, None, Some(1)) {
        if let Some(symbol_result) = symbol_results.first() {
            // Found the symbol, now trace its data flow
            trace_symbol_data_flow(server, &symbol_result.node, direction, max_depth, include_transformations).await
        } else {
            serde_json::json!({
...
async fn analyze_transitive_dependencies(server: &PrismMcpServer, arguments: Option<&Value>) -> Result<CallToolResult> {
    let args = arguments.ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
    ...
    // Try to resolve the symbol
    let result = if let Ok(symbol_results) = server.graph_query().search_symbols(target, None, Some(1)) {
        if let Some(symbol_result) = symbol_results.first() {
            // Found the symbol, now analyze its transitive dependencies
            analyze_symbol_transitive_dependencies(server, &symbol_result.node, max_depth, detect_cycles).await
        } else {
            serde_json::json!({
                "target": target,
                "error": "Symbol not found",
