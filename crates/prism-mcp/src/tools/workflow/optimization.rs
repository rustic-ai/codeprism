...
pub async fn optimize_workflow(
    ...
    let workflow_history = args.get("workflow_history")
    ...
    let optimization_goals = args.get("optimization_goals")
        .and_then(|v| v.as_array())
    ...
    let analysis_data = if let Some(history) = workflow_history {
        ...
        return Err(anyhow::anyhow!("Either workflow_history or session_id must be provided"));
    };
...
fn analyze_workflow_history(history: &[Value]) -> Result<WorkflowAnalysis> {
    ...
    for entry in history {
        let tool_name = entry.get("tool_name")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        
        let execution_time = entry.get("execution_time_ms")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        ...
        let successes = tool_success.entry(tool_name.to_string()).or_insert_with(|| (0, 0));
    ...
    let average_time = if total_tools > 0 { total_time as f64 / total_tools as f64 } else { 0.0 };
    let success_rate = if total_tools > 0 { successful_tools as f64 / total_tools as f64 } else { 0.0 };
    ...
    for (tool_name, times) in &tool_times {
        ...
        let tool_success_rate = if *total > 0 { *successes as f64 / *total as f64 } else { 0.0 };
        
        tool_usage_patterns.insert(tool_name.clone(), ToolUsagePattern {
...
fn identify_workflow_issues(
    ...
    for (tool_name, times) in tool_times {
        let avg_time = times.iter().sum::<u64>() as f64 / times.len() as f64;
        if avg_time > 15000.0 { // > 15 seconds
    ...
    for (tool_name, (successes, total)) in tool_success {
        ...
        if success_rate < 0.8 {
            issues.push(WorkflowIssue {
                ...
                description: format!("{} has low success rate: {:.1}%", tool_name, success_rate * 100.0),
                affected_tools: vec![tool_name.clone()],
...
fn estimate_parallelization_efficiency(tool_times: &HashMap<String, Vec<u64>>) -> f64 {
    ...
    let avg_times: Vec<f64> = tool_times.values()
...
fn generate_optimization_recommendations(
        ...
        for (tool_name, pattern) in &analysis.tool_usage_patterns {
            if pattern.average_execution_time_ms > 10000.0 {
                recommendations.push(OptimizationRecommendation {
                    ...
                    description: format!("Optimize {} parameters to reduce execution time", tool_name),
                ...
                quick_wins.push(format!("Optimize {} parameters for faster execution", tool_name));
            }
        }
    }
    ...
    if goals.contains(&"resource_usage") {
        advanced_optimizations.push("Implement intelligent caching for expensive operations".to_string());
    ...
    let time_reduction = if analysis.performance_metrics.parallelization_efficiency < 0.8 { 0.4 } else { 0.2 };
    ...
    let projected_metrics = PerformanceMetrics {
        total_execution_time_ms: (analysis.performance_metrics.total_execution_time_ms as f64 * (1.0 - time_reduction)) as u64,
        ...
        parallelization_efficiency: (analysis.performance_metrics.parallelization_efficiency + 0.3).min(1.0),
...
fn create_optimized_sequence(tool_patterns: &HashMap<String, ToolUsagePattern>) -> Result<Vec<OptimizedToolStep>> {
    ...
    if tool_patterns.contains_key("repository_stats") {
        sequence.push(OptimizedToolStep {
            ...
            expected_time_ms: tool_patterns.get("repository_stats")
    ...
    let analysis_tools = ["analyze_complexity", "analyze_security", "analyze_performance"];
    ...
    if sequence.iter().any(|s| s.parallel_group == Some(parallel_group_id)) {
        order += 1;
        parallel_group_id += 1;
    }
...
fn identify_parallel_groups(tool_patterns: &HashMap<String, ToolUsagePattern>) -> Vec<Vec<String>> {
    ...
    let analysis_tools: Vec<String> = ["analyze_complexity", "analyze_security", "analyze_performance"]
        .iter()
