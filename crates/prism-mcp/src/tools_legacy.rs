...
impl ToolManager {
    ...
    async fn trace_inheritance(&self, server: &PrismMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        ...
        for target_class in &target_classes {
            ...
            let metaclass_analysis = if include_metaclasses && inheritance_info.metaclass.is_some() {
                Some(self.analyze_metaclass_impact(server, &inheritance_info).await?)
            } else {
                None
            };
            ...
            let mixin_analysis = if include_mixins && !inheritance_info.mixins.is_empty() {
                Some(self.analyze_mixin_relationships(server, &inheritance_info).await?)
            } else {
                None
            };
            ...
            let mro_analysis = if include_mro && !inheritance_info.method_resolution_order.is_empty() {
                Some(self.analyze_method_resolution_order(server, &inheritance_info).await?)
            } else {
                None
            };
            ...
            let dynamic_attributes_analysis = if include_dynamic_attributes && !inheritance_info.dynamic_attributes.is_empty() {
                Some(self.analyze_dynamic_attributes(server, &inheritance_info).await?)
            } else {
                None
            };
            
            // Diamond inheritance detection
            let diamond_inheritance = self.detect_diamond_inheritance(server, &target_class.id).await?;
        ...
        if let Some(class_node) = server.graph_store().get_node(class_id) {
            if let Ok(inheritance_info) = server.graph_query().get_inheritance_info(class_id) {
                ...
                if include_source_context {
                    if let Some(context) = self.extract_source_context(&class_node.file, class_node.span.start_line, 3) {
            ...
            for i in 0..inheritance_info.base_classes.len() {
                for j in (i + 1)..inheritance_info.base_classes.len() {
                    ...
                    if let (Some(base1_node), Some(base2_node)) = (
                        all_classes.iter().find(|node| node.name == base1.class_name),
                        all_classes.iter().find(|node| node.name == base2.class_name)
                    ) {
                        if let (Ok(base1_info), Ok(base2_info)) = (
                            ...
                            let common_ancestors: Vec<_> = base1_info.inheritance_chain
                                .iter()
    ...
    async fn analyze_decorators(&self, server: &PrismMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        let args = arguments.ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
        ...
        for target_decorator in &target_decorators {
            ...
            let decorator_usage = self.analyze_decorator_usage(server, &target_decorator.id, scope).await?;
            
            // Decorator effects analysis
            let effects_analysis = if analyze_effects {
                Some(self.analyze_decorator_effects(server, &target_decorator.id).await?)
            } else {
                None
            };
            ...
            let factory_analysis = if include_factories {
                Some(self.analyze_decorator_factory(server, &target_decorator.id).await?)
            } else {
                None
            };
            ...
            let chain_analysis = if include_chains {
                Some(self.analyze_decorator_chains(server, &target_decorator.id).await?)
            } else {
                None
            };
            ...
            let framework_analysis = if include_framework_analysis {
                Some(self.analyze_framework_decorators(server, &target_decorator.id).await?)
            } else {
                None
            };
            ...
            let pattern_analysis = if detect_patterns {
                Some(self.detect_decorator_patterns(server, &target_decorator.id, confidence_threshold).await?)
            } else {
                None
            };
            ...
            // Add source context if requested
            if include_source_context {
                if let Some(context) = self.extract_source_context(&target_decorator.file, target_decorator.span.start_line, 3) {
    ...
    async fn analyze_framework_decorators(
        ...
        // FastAPI framework patterns
            ...
            framework_info.insert("pattern_type".to_string(), serde_json::Value::String("dependency_injection".to_string()));
        }
        ...
        // Generic patterns
            ...
            framework_info.insert("framework".to_string(), serde_json::Value::String("unknown".to_string()));
    ...
    async fn detect_decorator_patterns(
        ...
        // Caching pattern
        if decorator_name.contains("cache") || decorator_name.contains("memoize") || decorator_name.contains("lru") {
            detected_patterns.push(serde_json::json!({
        ...
        if decorator_name.contains("auth") || decorator_name.contains("require") || decorator_name.contains("permission") {
            detected_patterns.push(serde_json::json!({
        ...
        if decorator_name.contains("time") || decorator_name.contains("measure") || decorator_name.contains("profile") {
            detected_patterns.push(serde_json::json!({
    ...
    fn get_decorator_recommendations(&self, patterns: &[serde_json::Value]) -> Vec<String> {
        ...
        if patterns.is_empty() {
            recommendations.push("Consider adding more descriptive names to better identify decorator patterns".to_string());
            ...
            if patterns.iter().any(|p| p["pattern"].as_str().unwrap_or("").contains("Caching")) {
                recommendations.push("Consider cache invalidation strategy for caching decorators".to_string());
            }
            
            if patterns.iter().any(|p| p["pattern"].as_str().unwrap_or("").contains("Authorization")) {
                recommendations.push("Ensure authorization decorators are applied consistently across similar endpoints".to_string());
            }
            
            if patterns.iter().any(|p| p["pattern"].as_str().unwrap_or("").contains("Validation")) {
                recommendations.push("Consider combining validation patterns with error handling".to_string());
            }
    ...
    async fn find_unused_code(&self, server: &PrismMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        ...
        let analyze_types = args.get("analyze_types")
            ...
            .unwrap_or_else(|| vec!["functions".to_string(), "classes".to_string(), "variables".to_string(), "imports".to_string()]);
        ...
        let analysis_result = self.perform_unused_code_analysis(
            server,
            scope,
            include_dead_code,
            consider_external_apis,
    ...
    async fn analyze_security(&self, server: &PrismMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        ...
        let vulnerability_types = args.get("vulnerability_types")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect::<Vec<_>>())
            .unwrap_or_else(|| vec!["injection".to_string(), "authentication".to_string(), "authorization".to_string()]);
        ...
        let analysis_result = self.perform_security_analysis(
            server,
            scope,
            &vulnerability_types,
            severity_threshold,
    ...
    async fn analyze_performance(&self, server: &PrismMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        ...
        let analysis_types = args.get("analysis_types")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect::<Vec<_>>())
            .unwrap_or_else(|| vec!["time_complexity".to_string(), "memory_usage".to_string(), "hot_spots".to_string()]);
        ...
        let analysis_result = self.perform_performance_analysis(
            server,
            scope,
            &analysis_types,
            complexity_threshold,
    ...
    async fn analyze_api_surface(&self, server: &PrismMcpServer, arguments: Option<Value>) -> Result<CallToolResult> {
        ...
        let analysis_types = args.get("analysis_types")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect::<Vec<_>>())
            .unwrap_or_else(|| vec!["public_api".to_string(), "versioning".to_string(), "breaking_changes".to_string()]);
        ...
        let analysis_result = self.perform_api_surface_analysis(
            server,
            scope,
            &analysis_types,
            api_version,
