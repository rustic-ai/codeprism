...
impl TestConfig {
    ...
    pub fn validate<P: AsRef<Path>>(path: P) -> Result<()> {
        ...
        if config.test_suites.is_empty() {
            return Err(anyhow!("Configuration must contain at least one test suite"));
        }
        
        for suite in &config.test_suites {
            if suite.test_cases.is_empty() {
                return Err(anyhow!("Test suite '{}' must contain at least one test case", suite.name));
            }
            
            for test_case in &suite.test_cases {
                if test_case.tool_name.is_empty() {
                    return Err(anyhow!("Test case '{}' must specify a tool_name", test_case.id));
                }
            }
        }
...
fn generate_database_template() -> TestConfig {
    ...
    config.server.env = Some({
        ...
        env.insert("DATABASE_URL".to_string(), "sqlite:///tmp/test.db".to_string());
        env
    });
