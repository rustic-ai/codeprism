...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Template {
            server_type,
            output,
        } => generate_template(server_type, output).await,
        Commands::List { server_type } => list_templates(server_type).await,
        Commands::Discover {
            port_range,
            timeout,
        } => discover_servers(port_range, timeout).await,
    }
}
...
async fn run_tests(
    ...
    info!(
        "Loaded configuration with {} test suites",
    ...
    match output_format {
        ...
        _ => {
            warn!(
                "Unknown output format '{}', using table format",
                output_format
            );
