...
impl McpServer {
        ...
        if let Some(mut process) = self.process.take() {
            match process.try_wait() {
                ...
                Ok(None) => {
                    ...
                    process.kill().await.context("Failed to kill server process")?;
                    process.wait().await.context("Failed to wait for process termination")?;
                }
                Err(e) => {
    ...
    pub async fn call_tool(&mut self, tool_name: &str, arguments: Option<serde_json::Value>) -> Result<serde_json::Value> {
    ...
    pub async fn get_prompt(&mut self, prompt_name: &str, arguments: Option<serde_json::Value>) -> Result<serde_json::Value> {
        let params = serde_json::json!({
            "name": prompt_name,
            "arguments": arguments.unwrap_or(serde_json::json!({}))
        });
...
impl StdioClient {
    ...
    async fn handle_response(
        ...
        let request_id = response.id.as_str()
